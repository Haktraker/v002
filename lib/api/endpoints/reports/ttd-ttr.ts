...
export const TTD_TTR_KEYS = {
  lists: (params?: TtdTtrQueryParams) => ['reports-ttdTtr', params] as const,
  detail: (id: string) => ['reports-ttdTtr', id] as const,
};

// Base URL
const BASE_URL = '/reports/ttd-ttr';
...
export const useCreateTtdTtrRecord = () => {
  ...
  return useMutation<TtdTtr, Error, CreateTtdTtrDto>({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: [TTD_TTR_KEYS.lists()[0]] });
    },
    onError: (error: any) => {
