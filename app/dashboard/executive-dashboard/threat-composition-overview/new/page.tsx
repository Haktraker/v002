...
import { useCreateThreatCompositionOverview } from '@/lib/api/endpoints/executive-dashboard/threat-composition-overview';
...
import { CreateThreatCompositionOverviewDto, ThreatCompositionOverviewType } from '@/lib/api/executive-dashboard-types/types';
...
// Define the possible threat types based on the type definition
const threatTypes: ThreatCompositionOverviewType[] = [
  "ATO",
  "Insider Threats",
  "Trojan and Malware",
  "3rd Party leaks",
...
export default function NewThreatCompositionOverviewPage() {
  ...
  const createRecord = useCreateThreatCompositionOverview();
  const { withLoading } = useApiLoading();

  // State for single form
  const [formData, setFormData] = useState<CreateThreatCompositionOverviewDto>({
    month: '',
    year: '',
    score: '',
    threatType: threatTypes[0], // Default to the first type
    quarter: 0,
  });

  // Validation logic for CSV rows
  const validateRow = (row: CreateThreatCompositionOverviewDto): { isValid: boolean, error?: string } => {
    if (!row.month || !row.year || !row.score || !row.threatType || row.quarter === undefined) {
      return { isValid: false, error: 'Missing required fields in a row.' };
    }
    if (!threatTypes.includes(row.threatType)) {
      return { isValid: false, error: `Invalid threat type: ${row.threatType}` };
    }
    ...
    // Add more validations (e.g., score format, year/month format)
    return { isValid: true };
  };
  ...
  const {
    ...
    sortConfig,
    handleSort,
  } = useTableData<CreateThreatCompositionOverviewDto>({
    requiredFields: ['month', 'year', 'score', 'threatType', 'quarter'],
    validateRow,
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await withLoading(async () => {
        ...
        toast.success('Threat Composition Overview record created successfully');
        router.push('/dashboard/executive-dashboard/threat-composition-overview');
      });
    } catch (error) {
    ...
    <div className="p-6">
      <Breadcrumb>
        <BreadcrumbList>
          ...
          <BreadcrumbSeparator />
          <BreadcrumbItem>
            <BreadcrumbLink href="/dashboard/executive-dashboard/threat-composition-overview">Threat Composition Overview</BreadcrumbLink>
          </BreadcrumbItem>
          <BreadcrumbSeparator />
          <BreadcrumbItem>
             <BreadcrumbPage>Add New Record</BreadcrumbPage>
          </BreadcrumbItem>
        </BreadcrumbList>
      </Breadcrumb>
      ...
      <div className="flex items-center gap-4 my-6">
        ...
        <h1 className="text-2xl font-semibold">Add New Threat Composition Overview Record</h1>
      </div>
          ...
          <Card>
            <form onSubmit={handleSubmit}>
              <CardHeader>
                ...
                <CardDescription>
                  Enter the details for a single record.
                </CardDescription>
              </CardHeader>
              <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-4">
                 <div className="space-y-2">
                  ...
                  <Select
                    ...
                    onValueChange={(value) => setFormData({ ...formData, threatType: value as ThreatCompositionOverviewType })}
                    required
                  >
                    <SelectTrigger id="threatType">
                ...
                <div className="space-y-2">
                  ...
                  <Input id="score" value={formData.score} onChange={(e) => setFormData({ ...formData, score: e.target.value })} placeholder="e.g., 75" required />
                </div>
                 <div className="space-y-2">
        ...
        <TabsContent value="bulk">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Upload a CSV file. Required columns: month, year, score, threatType, quarter.
              </CardDescription>
            </CardHeader>
            <CardContent>
                    ...
                    <div className="border rounded-lg overflow-hidden">
                      <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        <thead className="bg-gray-50 dark:bg-gray-800">
                          <tr>
                             {['threatType', 'score', 'month', 'year', 'quarter'].map(header => (
                        ...
                        <tbody className="bg-white dark:bg-gray-900 divide-y divide-gray-200 dark:divide-gray-700">
                          {currentPageData.map((row, index) => (
                            <tr key={index}>
                              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">{row.threatType}</td>
