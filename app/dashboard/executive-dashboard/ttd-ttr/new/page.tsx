...
import { useCreateTtdTtrRecord } from '@/lib/api/endpoints/executive-dashboard/ttd-ttr';
...
import { CreateTtdTtrDto, TtdTtrIndicator } from '@/lib/api/executive-dashboard-types/types';
...
export default function NewTtdTtrPage() {
  ...
  const validateRow = (row: CreateTtdTtrDto): { isValid: boolean, error?: string } => {
    if (!row.indicator || !row.score || !row.month || !row.year || !row.quarter) {
      return { isValid: false, error: 'Missing required fields: indicator, score, month, year, quarter.' };
    }
  ...
  const {
    ...
    requiredFields: ['indicator', 'score', 'month', 'year', 'quarter'],
    validateRow,
  });
      ...
      await withLoading(async () => {
        for (const row of csvData) {
          try {
            ...
            errorCount++;
            errorMessages.push(err?.response?.data?.message || `Failed for ${row.indicator} in Q${row.quarter}/${row.year}`);
          }
        }
        if (successCount > 0) {
    ...
    <div className="p-6">
      <Breadcrumb>
        <BreadcrumbList>
          ...
          <BreadcrumbSeparator />
          <BreadcrumbItem>
            <BreadcrumbLink href="/dashboard/executive-dashboard/ttd-ttr">TTD/TTR</BreadcrumbLink>
          </BreadcrumbItem>
          <BreadcrumbSeparator />
          <BreadcrumbItem>
            ...
            <form onSubmit={handleSubmit}>
              ...
              <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                  ...
                  <Input id="quarter" name="quarter" type="number" min="1" max="4" value={formData.quarter || ''} onChange={handleInputChange} placeholder="e.g., 3" required />
                </div>
              </CardContent>
              <CardFooter>
        ...
        <TabsContent value="bulk">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Upload a CSV file. Required columns: indicator, score, month (full name, e.g., January), year, quarter.<br />
