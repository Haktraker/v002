...
import { useCreateThirdPartyThreat } from '@/lib/api/endpoints/executive-dashboard/third-party-threat-intelligence';
...
import { CreateThirdPartyThreatIntelligenceDto, SeverityLevel } from '@/lib/api/executive-dashboard-types/types';
...
export default function NewThirdPartyThreatPage() {
  ...
  const [formData, setFormData] = useState<CreateThirdPartyThreatIntelligenceDto>({
    thirdParty: '',
  ...
  const validateRow = (row: CreateThirdPartyThreatIntelligenceDto): { isValid: boolean, error?: string } => {
    if (!row.thirdParty || !row.severity || !row.month || !row.year || !row.quarter) {
      return { isValid: false, error: 'Missing required fields: thirdParty, severity, month, year, quarter.' };
    }
    ...
    <div className="p-6">
      <Breadcrumb>
        <BreadcrumbList>
          ...
          <BreadcrumbSeparator />
          <BreadcrumbItem>
            <BreadcrumbLink href="/dashboard/executive-dashboard/third-party-threat-intelligence">Third Party Threats</BreadcrumbLink>
          </BreadcrumbItem>
          <BreadcrumbSeparator />
          <BreadcrumbItem>
      ...
      <div className="flex items-center gap-4 my-6">
        <Link href="/dashboard/executive-dashboard/third-party-threat-intelligence">
